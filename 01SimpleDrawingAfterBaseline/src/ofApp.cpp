...
void ofApp::draw(){
    ...
    if(baselineCount >= 10000){
        baseline = baselineSum/10000;
    }
    int differenceGsr = value - baseline;
    //reference line
    ofSetColor(200);
    ofDrawRectangle(ofGetWidth()/2 - 100, ofGetHeight()/2 - 1, 200, 2);
    //bargraph
    
    ofSetColor(differenceGsr, 150, 255 - differenceGsr);
    ofDrawRectangle(ofGetWidth()/2 - 50, ofGetHeight()/2, 100, -2 * differenceGsr);
    ...
    ofDrawBitmapString(baselineCount, 50, 300); // Write the data on the output window
    ofDrawBitmapString(baseline, 100, 300); // Write the data on the output window
